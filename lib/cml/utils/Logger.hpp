...
public:
    ...
    template<typename... params> uint32_t inf(const char* a_p_format, params... a_params)
    {
        if (true == this->is_stream_enabled(Stream_type::inf))
        {
            ...
            uint32_t length = common::cstring::format(
                this->line_buffer + 6, config::logger::line_buffer_capacity - 6, a_p_format, a_params...);
...
private:
    ...
    {
        common::memory::copy(
            this->line_buffer, config::logger::line_buffer_capacity, tags[static_cast<uint32_t>(a_type)], 6);

        uint32_t length =
            common::memory::copy(this->line_buffer + 6,
                                 config::logger::line_buffer_capacity - 6,
                                 a_p_message,
                                 common::cstring::length(a_p_message, config::logger::line_buffer_capacity - 6));
    ...
    {
        return (true == a_inf ? 0x1u : 0x0u) | (true == a_wrn ? 0x2u : 0x0u) | (true == a_err ? 0x4u : 0x0u) |
