...
int main()
{
    ...
    if (mcu::Sysclk_source::hsi == mcu::get_sysclk_source())
    {
        ...
        if (true == usart_ready)
        {
            ...
            console.write_line("CML Console SLAVE. CPU speed: %u MHz", mcu::get_sysclk_frequency_hz() / MHz(1));
            ...
            while (true)
            {
                ...
                //for (uint8 i = 0; i < 3; i++)
                {
                    uint16 data[] = { 0x0u, 0x0u, 0x0u };
                    //uint32 data = cml::numeric_traits<uint32>::get_max();
                    auto[f, words] = interlink_usart.receive_bytes_polling(data, sizeof(data) / sizeof(data[0]));
                    ...
                    {
                        console.write_line("%u, v: %u %u %u", words, data[0], data[1], data[2]);
                    }
