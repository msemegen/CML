...
int main()
{
    ...
    if (mcu::Sysclk_source::hsi == mcu::get_sysclk_source())
    {
        ...
        pin::af::Config i2c_pin_config = { pin::Mode::open_drain, pin::Pull::up, pin::Speed::high, 0x4u };
        ...
        if (true == usart_ready)
        {
            ...
            if (true == i2c_master_bus.is_slave_connected(0x11, 10))
            {
                ...
                while (true)
                {
                    auto result = i2c_master_bus.transmit_bytes_polling(0x11, data_to_send, sizeof(data_to_send), 10);
